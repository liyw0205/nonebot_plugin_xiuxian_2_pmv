...
保证 exactly-once，重复回调即使再次进入处理链也不会重复调用平台 ACK。

QQ 好友、C2C 和群生命周期事件已接入 `LifecycleStateRegistry`。群状态按 AppID 和
`group_openid` 隔离，记录 Bot 是否仍在群内、群消息接收能力以及各 action 的事件
计数。预处理结果会放入事件的 `xiuxian_lifecycle_context` 和
