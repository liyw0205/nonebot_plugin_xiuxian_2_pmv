...
执行 DDL；攻击力增加、道具扣除/绑定数、耐药计数、重复请求、BuffInfo/库存缺失和事务回滚语义保持
原子一致。新增 application/repository、wiring、source/progress 回归；聚焦回归与永久攻击丹旧语义测试
...
2026-09-26 rift treasure outcome cutover：真实普通秘境宝物事件改经
...
2026-09-26 rift Boss item lookup provider：Rift 玩家战斗快照 provider 将
...
2026-09-26 rift Boss final-attribute provider：Rift 玩家战斗快照 provider 显式注入
...
2026-09-26 auction startup reconciliation cutover：重启对账默认入口改由 trade facade 的 `_reconcile_auction_with_application` 编排，只读取 feature-owned query/session application；未到结束时间继续等待，已到结束时间使用稳定 operation ID 调用 `AuctionSettlementApplication`，空场次和缺失 active session 语义保持不变。旧 `reconcile_auction_after_restart` 仍仅作为显式兼容入口，未从 transaction service 删除。拍卖/交易聚焦回归 `135 passed`，compileall、architecture、progress、inventory 和 diff check 通过；缓存与临时产物已清理，用户原有 Boss JSON 改动保留。下一步处理交易兼容 adapter 的真实调用归零证明，再选择签到副作用或背包/宠物等高频资产切片。

2026-09-26 auction queue compatibility isolation：`AuctionQueueService` 与结果 DTO 从 `xiuxian_trade/transaction_service.py` 移至 `compatibility/legacy_trade_auction_queue.py`，旧 `transaction_service.AuctionQueueService` import 保持 re-export 兼容；`auction_queue_service.py` 也改为直接指向 compatibility-only 模块。真实拍卖命令继续只调用 `AuctionQueueApplication`，未改变队列事务、跨库 attach、库存扣减、限额和 replay 语义。拍卖/交易回归 `138 passed`，窄来源质量回归 `22 passed`，compileall、architecture、progress、inventory 和 diff check 通过；旧 transaction service 减少 `83` 行、旧 service import 命中减少 `1`。下一步按 6.2.5 选择签到副作用或背包礼包/通用物品的下一条高频旧兼容路径。

2026-09-26 back alchemy compatibility isolation：旧 `AlchemyService` 与 `AlchemyResult` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_alchemy.py`，旧 transaction service import 保持 re-export 兼容；默认 `xiuxian_back` facade 与 `LegacyBackRepository` 的显式回滚分支均直接指向 compatibility-only 模块。保留请求期 `alchemy_operations` schema、`BEGIN IMMEDIATE`、批量扣除、已装备数量保护、灵石入账、duplicate/conflict 和 rollback 语义；默认 handler 继续只调用 `BackApplication.alchemy`。炼金窄回归 `11 passed`，背包相关回归除 2 条既有无关基线失败外通过，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步继续按 6.2.5 处理背包其它高频兼容资产或签到副作用，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back package-reward compatibility isolation：旧 `PackageRewardService`、`PackageReward` 与 `PackageOpenResult` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_package_reward.py`，旧 import 保持 re-export 兼容；礼包 compatibility facade 的显式环境变量 rollback、`LegacyBackRepository` 回滚分支均直接指向 compatibility-only 模块。保留请求期 `package_reward_operations` schema、`BEGIN IMMEDIATE`、绑定数量保护、灵石/物品奖励、库存上限、duplicate/state_changed 和 rollback 语义；默认礼包 handler 继续只调用 `PackageRewardApplication`。礼包/背包窄回归 `20 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步继续按 6.2.5 处理背包其它高频兼容资产或签到副作用，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back pet-egg compatibility isolation：旧 `BatchItemUseService` 包装从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_pet_egg.py`，旧 import 保持 re-export 兼容；`xiuxian_back` facade 与 `LegacyBackRepository` 显式回滚路径直接引用 compatibility-only 包装。保留 `PetEggUseSqlRepository` 的 game/player attached UoW、请求期 schema、宠物快照、容量限制、duplicate/operation conflict 和 rollback 语义；真实宠物蛋与通用物品路径继续分别由 `PetEggApplication`、`ItemUseApplication` 承载。宠物蛋/通用物品窄回归 `21 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步继续处理背包 accessory 兼容边界或签到副作用，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。
...
2026-09-26 back skill-learning compatibility isolation：旧 `SkillLearningService`、`SkillLearningResult` 与 `SKILL_COLUMNS` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_skill_learning.py`，旧 import 保持 re-export 兼容；背包 facade、service facade 和 `LegacyBackRepository` 显式回滚分支直接引用 compatibility-only 实现。默认学功法 handler 继续走 `BackApplication.learn_skill -> SkillLearningApplication -> SkillLearningSqlRepository`，保留技能槽位、绑定数量、operation replay 和 rollback 语义。技能学习/应用/source 窄回归 `12 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步按优先级处理 `LotteryTalismanService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back lottery-talisman compatibility isolation：旧 `LotteryTalismanService`、`LotteryTalismanUse` 与 `LotteryReward` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_lottery_talisman.py`，旧 import 保持 re-export 兼容；背包 facade 直接引用 compatibility-only 实现。默认灵签宝箓 handler 继续走 `BackApplication.lottery_talisman -> LotteryTalismanApplication -> LotteryTalismanSqlRepository`，保留奖励合并、库存上限、operation replay 和 rollback 语义。Lottery 宝箓/应用/source 窄回归 `13 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步按优先级处理 `StoneItemRewardService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back stone-reward compatibility isolation：旧 `StoneItemRewardService` 与 `StoneItemReward` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_stone_reward.py`，旧 import 保持 re-export 兼容；背包 facade 直接引用 compatibility-only 实现。默认灵石袋/天机石 handler 继续走 `BackApplication.stone_reward -> StoneRewardApplication -> StoneRewardSqlRepository`，保留预滚奖励、库存扣除、灵石入账、operation replay 和 rollback 语义。Stone reward/应用/source 窄回归 `12 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步按优先级处理 `ThreeCultivationPillService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back three-cultivation-pill compatibility isolation：旧 `ThreeCultivationPillService` 与 `ThreeCultivationPillUse` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_three_cultivation_pill.py`，旧 import 保持 re-export 兼容；背包 facade 直接引用 compatibility-only 实现。默认三转玄丹 handler 继续走 `BackApplication.three_cultivation_pill -> ThreeCultivationPillApplication -> ThreeCultivationPillSqlRepository`，保留经验上限、HP/MP 恢复、功力更新、库存绑定数量、operation replay 和 rollback 语义。三修丹/应用/source 窄回归 `15 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步按优先级处理 `UnbindItemService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back unbind compatibility isolation：旧 `UnbindItemService` 与 `UnbindItemResult` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_unbind.py`，旧 import 保持 re-export 兼容；背包 facade、service facade 和 `LegacyBackRepository` 显式回滚分支直接引用 compatibility-only 实现。默认解绑符 handler 继续走 `BackApplication.unbind -> UnbindApplication -> UnbindSqlRepository`，保留数量截断、绑定数 CAS、operation replay 和 rollback 语义。解绑/应用/source 窄回归 `12 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步按优先级处理 `CultivationItemService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back cultivation-item compatibility isolation：旧 `CultivationItemUse` 与 `CultivationItemService` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_cultivation_item.py`，旧 import 保持 re-export 兼容；背包 facade、service facade、`back_util` 与 `LegacyBackRepository` 显式回滚分支直接引用 compatibility-only 实现。默认神物/丹药经验入口继续走 `BackApplication.cultivation_item -> CultivationItemApplication -> CultivationItemSqlRepository`，保留经验、气血、灵力、攻击、power、耐药计数、绑定数量扣除、operation replay 和事务 rollback 语义。cultivation-item/应用/source 窄回归 `12 passed`，compileall、architecture、progress、inventory 和 diff check 通过；本阶段未新增 migration，缓存与临时目录已清理。下一步按 transaction service 顺序处理 `BreakthroughRateItemService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back breakthrough-rate-item compatibility isolation：旧 `BreakthroughRateItemUse` 与 `BreakthroughRateItemService` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_breakthrough_rate_item.py`，旧 import 保持 re-export 兼容；`back_util`、service facade 与 `LegacyBackRepository` 显式兼容回滚路径直接引用 compatibility-only 实现。默认成功率丹入口继续走 `BreakthroughRateItemApplication -> BreakthroughRateItemSqlRepository`，保留成功率增量、耐药计数、绑定数量扣除、operation replay 和事务 rollback 语义；同时清除上一轮抽取遗留的不可达重复异常分支。突破率丹药/source 窄回归 `7 passed`，compileall、architecture、progress、inventory 和 diff check 通过；旧 `transaction_service.py` 减少 `127` 行，本阶段未新增 migration，缓存与临时目录已清理。下一步按 transaction service 顺序处理 `PermanentAtkItemService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back permanent-atk-item compatibility isolation：旧 `PermanentAtkItemUse` 与 `PermanentAtkItemService` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_permanent_atk_item.py`，旧 import 保持 re-export 兼容；`back_util`、service facade 与 `LegacyBackRepository` 显式兼容回滚路径直接引用 compatibility-only 实现。默认攻击丹入口继续走 `PermanentAtkItemApplication -> PermanentAtkItemSqlRepository`，保留 BuffInfo 攻击更新、耐药计数、绑定数量扣除、operation replay 和事务 rollback 语义。攻击丹药/source 窄回归 `7 passed`，compileall、architecture、progress、inventory 和 diff check 通过；旧 `transaction_service.py` 减少 `114` 行，本阶段未新增 migration，缓存与临时目录已清理。下一步按 transaction service 顺序处理 `RecoveryItemService`，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back recovery-item compatibility isolation：旧 `RecoveryItemUse` 与 `RecoveryItemService` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_recovery_item.py`，旧 import 保持 re-export 兼容；`back_util`、service facade 与 `LegacyBackRepository` 显式兼容回滚路径直接引用 compatibility-only 实现。默认恢复丹入口继续走 `RecoveryItemApplication -> RecoveryItemSqlRepository`，保留 HP/MP 恢复、全恢复攻击重算、体力上限、耐药计数、绑定数量扣除、operation replay 和事务 rollback 语义。恢复丹/source 窄回归 `8 passed`，compileall、architecture、progress、inventory 和 diff check 通过；旧 `transaction_service.py` 减少 `188` 行，本阶段未新增 migration，缓存与临时目录已清理。下一步处理剩余的 `BlessedFlagReplaceService` compatibility 边界，并保留全局 legacy transaction service/`xiuxian2_handle` 未完成状态。

2026-09-26 back blessed-flag-replace compatibility isolation：旧 `BlessedFlagReplaceResult` 与 `BlessedFlagReplaceService` 从 `xiuxian_back/transaction_service.py` 移至 `compatibility/legacy_back_blessed_flag_replace.py`，旧 import 保持 re-export 兼容；`back_util` 与 `LegacyBackRepository` 的显式兼容回滚路径直接引用 compatibility-only 实现，并保留双数据库构造、attached transaction 和跨库 rollback。默认聚灵旗替换入口继续走 `BlessedFlagReplaceApplication -> BlessedFlagReplaceSqlRepository`；补齐旧结果 DTO 缺失的 `@dataclass(frozen=True)`，原旧服务回归从基线 `5 failed` 修复为完整通过。旧服务/application/source 窄回归 `13 passed`，compileall、architecture、progress、inventory 和 diff check 通过；旧 `transaction_service.py` 减少 `191` 行，本阶段未新增 migration，缓存与临时目录已清理。背包中仍有 legacy repository fallback/import 和 compatibility re-export；全局 blockers 仍是 legacy transaction services 与 `xiuxian2_handle` legacy execution paths。

2026-09-26 back default compatibility repository removal：`BackApplication` 默认不再构造 `LegacyBackRepository`，仅保留显式注入作为 rollback 路径；feature-owned action 继续使用各自 application，旧 compatibility adapter 不再挂在默认应用对象上。补充默认 `repository is None` 回归，并为显式 rollback 测试夹具初始化 platform operation schema。背包 application/package/accessory/blessed-flag 聚焦回归 `29 passed`，显式 rollback unittest `1 passed`；architecture、inventory、compileall 与 diff check 通过，无 migration。工作树原本无 Python/pytest 缓存及 SQLite sidecar；本轮 pytest 临时目录、Python/pytest 缓存和字节码均已清理并复核；下一步审计饰品旧命令对 `AccessoryTransactionService` 的真实调用，按单个资产动作选择 feature-owned 切片。全局 legacy transaction services 与 `xiuxian2_handle` blockers 保持未完成。

2026-09-26 back accessory affix lock/unlock cutover：饰品锁定、解锁两个真实 matcher 从 `AccessoryTransactionService.set_affix_locks` 切换到 `AccessoryAffixApplication -> AccessoryAffixSqlRepository`；锁定/解锁规则进入纯 domain plan，玩家 `player_accessory` 通过 attached UoW 与 game-db `accessory_transaction_operations` 同事务写入。新增 `back.017` 启动迁移，仅路由 game DB；旧服务仍保留洗练、分解、升阶、预设等未迁移动作的显式兼容边界，旧锁定/解锁调用从真实 handler 消失。覆盖 applied/duplicate、operation payload conflict、过期快照、无饰品/无词条、触发器异常跨库回滚、请求期不建表、旧 operation 表兼容和迁移路由；饰品/背包/source 聚焦回归 `40 passed`，隔离 recovery smoke 完成五库 backup、restore dry-run/restore、全量迁移，`back.017` 仅在 game DB，reconcile `clean=true` 且 operations/outbox/dead events 均为 `0`。architecture、progress、inventory、compileall 与 diff check 通过；本轮临时数据、receipt、pytest/字节码缓存已清理后再提交。下一步继续审计饰品洗练或分解的真实调用图，并保留全局 legacy transaction services 与 `xiuxian2_handle` blockers。

2026-09-26 back accessory single-decompose cutover：单件 `饰品分解` 真实 matcher 从 `AccessoryTransactionService.decompose` 切换到 `AccessoryDecomposeApplication -> AccessoryDecomposeSqlRepository`；保留只允许背包饰品、洗练石库存上限、绑定数量、expected snapshot、重复请求和跨 game/player 事务回滚语义。复用已启动的 `back.017` operation 表，不新增 migration；快速分解批处理、洗练、升阶和预设继续保留显式兼容路径。覆盖 applied/duplicate、payload/snapshot conflict、inventory full、operation insert failure rollback、请求期不建表和真实 handler source contract；饰品/背包/source 聚焦回归 `44 passed`，隔离 recovery smoke 五库恢复与 reconcile clean（operations/outbox/dead events=`0`）通过。progress、architecture、inventory、compileall 和 diff check 通过；缓存、字节码、临时数据与 receipt 清理后提交。下一步审计饰品快速分解批处理或洗练随机边界，继续保留全局 legacy transaction services 与 `xiuxian2_handle` blockers。

2026-09-26 back accessory batch-decompose cutover：`快速分解饰品` 真实 matcher 也切换到 `AccessoryDecomposeApplication.batch_decompose -> AccessoryDecomposeSqlRepository`；handler 继续负责类型/品阶筛选和全部/高阶保护，repository 负责整包 expected snapshot、批量 UID 完整性、洗练石库存上限、跨库原子删除/入账和 `batch_decompose` operation replay/conflict。快速分解不新增 migration，仍复用 `back.017`；洗练、升阶、预设保留 compatibility-only 路径。饰品分解/锁定/source 聚焦回归 `42 passed`，覆盖批量 applied/duplicate、整包顺序冲突、库存不足、真实 handler application-owned；恢复 smoke reconcile clean 且 operations/outbox/dead events 均为 `0`，progress、architecture、inventory、compileall 和 diff check 通过。缓存、字节码、临时数据和 receipt 清理后提交。下一步审计饰品洗练随机边界，继续保留全局 legacy transaction services 与 `xiuxian2_handle` blockers。

2026-09-26 back accessory-wash cutover：饰品洗练真实 matcher 从 `AccessoryTransactionService.wash` 切换到 `AccessoryWashApplication -> AccessoryWashSqlRepository`；词条重掷、锁定词条保留、150 次保底和随机源注入进入纯 domain，repository 负责洗练石扣除、玩家附件更新、expected snapshot、operation replay/conflict 与跨库原子回滚。复用既有 `back.017` operation 表，不新增 migration；升阶、预设仍保留显式 compatibility-only 路径。饰品洗练/分解/锁定/source 聚焦回归 `51 passed`，compileall、architecture、progress、inventory 和 diff check 通过；pytest 临时目录、字节码、`.pytest_cache`、SQLite sidecar 已清理。全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成。

2026-09-26 back accessory-upgrade cutover：饰品升阶真实 matcher 从 `AccessoryTransactionService.upgrade` 切换到 `AccessoryUpgradeApplication -> AccessoryUpgradeSqlRepository`；domain 负责品阶上限、材料数量/同阶同款校验、词条补齐与洗练次数重置，repository 负责 equipped/bag 全量快照、材料 UID 完整性、跨库原子消费、operation replay/conflict 与回滚。复用既有 `back.017` operation 表，不新增 migration；预设保存和快速装备仍保留显式 compatibility-only 路径。饰品升阶/洗练/分解/锁定/source 聚焦回归 `55 passed`，compileall、architecture、progress、inventory 和 diff check 通过；临时测试目录、字节码和 pytest 缓存已清理。全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成。

2026-09-26 back accessory-preset-save cutover：饰品预设保存真实 matcher 从 `AccessoryTransactionService.save_preset` 切换到 `AccessoryPresetApplication -> AccessoryPresetSqlRepository`；domain 负责预设索引与装备 UID 快照，repository 负责 preset/equipped CAS、覆盖标记、operation replay/conflict、空用户建行和 attached 跨库原子回滚。新增 attached migration `accessory_package.player_data.003`，仅补齐 `player_accessory.preset_1/2/3`，请求路径缺 schema 时返回 `schema_missing`，不隐式建列；快速装备仍保留显式 compatibility-only 路径。预设/attached/饰品/source/release-gate 聚焦回归 `69 passed`，compileall、architecture、progress、inventory、diff check 和隔离 recovery smoke 通过；五库 backup/restore 成功，attached 三项 migration、reconcile `clean=true`、operations/outbox/dead events 均为 `0`。缓存、临时 recovery 数据、receipt、字节码和 SQLite sidecar 已清理。全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成。

2026-09-26 back accessory-quick-equip cutover：快速装备饰品真实 matcher 从 `AccessoryTransactionService.quick_equip_preset` 切换到 `AccessoryQuickEquipApplication -> AccessoryQuickEquipSqlRepository`；domain 负责预设装备规划、部位不匹配跳过和失效 UID 清理，repository 负责 equipped/bag/preset 原子更新、expected snapshot、operation replay/conflict 与 attached 跨库回滚。复用既有 `back.017` operation 表和 `accessory_package.player_data.003`，不新增 migration；旧快速装备入口仅保留显式 compatibility-only 兼容路径。快速装备/饰品/source 聚焦回归 `73 passed`，compileall、architecture、progress、inventory、diff check 和隔离 recovery smoke 通过；五库 backup/restore 成功，attached 三项 migration、reconcile `clean=true`、operations/outbox/dead events 均为 `0`。本轮缓存、字节码、pytest 临时目录、SQLite sidecar 与临时 recovery 产物已清理。全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成，下一步继续处理下一个高频兼容动作。

2026-09-26 sign-in lottery scheduler cutover：每日 `daily_reset_lottery` 继续由 scheduler 触发，但重置边界改为调用 feature-owned `LotteryApplication.snapshot`，不再引用已移除的旧 `lottery_settlement_service` singleton；snapshot 只读已迁移 schema，不在任务路径建表或写入参与者。新增进度门禁 `lottery_scheduler_application_owned`/`legacy_lottery_scheduler_disabled`，签到/鸿运聚焦回归 `20 passed`，compileall、architecture、progress、inventory、diff check 和五库 recovery 通过；无新增 migration，reconcile `clean=true` 且 operations/outbox/dead events 均为 `0`。本阶段临时测试、恢复数据、receipt、字节码和 SQLite sidecar 已清理。旧 `LotterySettlementService` 仍保留为显式兼容实现，下一步隔离其实现归属后继续处理 base 兼容服务。

2026-09-26 sign-in lottery compatibility isolation：旧 `LotterySettlementService`、`LotterySettlementResult`、`LotteryPoolSnapshot` 与 `LotteryWinner` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_lottery.py`；旧 transaction service 只保留 re-export，显式回滚和历史 JSON 一次性迁移行为保持不变。新增 `lottery_service_isolated` 进度门禁，lottery/sign-in/source 聚焦回归 `20 passed`，compileall、architecture、progress、inventory、diff check 和五库 recovery 通过；旧 transaction service 减少约 `469` 行，无新增 migration。本轮 pytest/recovery/receipt、字节码和 SQLite sidecar 已清理。签到默认 handler、scheduler 和 feature application 已完成，下一步按 base 兼容服务顺序处理剩余高频资产动作。

2026-09-26 base player-rename compatibility isolation：旧 `PlayerRenameService` 与 `PlayerRenameResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_player_rename.py`；旧 import 保持 re-export，改名/改灵根的显式回滚行为、物品/灵石扣除、唯一性检查、operation replay 和事务回滚保持不变。新增 `base.rename_service_isolated` 进度门禁，改名/base/lottery 聚焦回归 `20 passed`，compileall、architecture、progress、inventory、diff check 和五库 recovery 通过；旧 transaction service 再减少约 `189` 行，无新增 migration。本轮测试、恢复数据、receipt、字节码和 SQLite sidecar 已清理。下一步继续处理 base transaction service 中的灵石/仙缘等剩余兼容服务。

2026-09-26 base stone-contest compatibility isolation：旧 `StoneContestService`、`StoneContestResult` 与 `StoneTheftResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_stone_contest.py`；`stone_contest_service.py` 稳定 facade、旧 transaction import 和 base repository 继续 re-export/显式兼容，contest/theft ledger、原子转账、体力扣除、operation replay/conflict 与 rollback 语义保持不变。新增 `base.stone_contest_service_isolated` 进度门禁，stone contest/theft、rename、lottery、progress 和 source 聚焦回归 `263 passed`（source quality 中已有 rift construction 断言失败，与本切片无关），compileall、inventory、diff check 通过；旧 transaction service 再减少约 `342` 行，无新增 migration。本轮 pytest 临时目录、字节码和 SQLite sidecar 已清理。下一步继续隔离 `StoneRobberySettlementService` 与 `XiangyuanSettlementService`。

2026-09-26 base stone-robbery compatibility isolation：旧 `StoneRobberySettlementService` 与 `StoneRobberyResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_stone_robbery.py`；`stone_robbery_service.py` 稳定 facade、旧 transaction import 和 base repository 继续 re-export/显式兼容，跨库 `ATTACH DATABASE`、combat snapshot、HP/MP/stamina/stone/statistics 原子结算、operation replay/conflict 与双库 rollback 语义保持不变。新增 `base.stone_robbery_service_isolated` 进度门禁，stone robbery/contest、rename、lottery、progress 和 source 聚焦回归 `31 passed`，compileall、architecture、inventory、diff check 通过；旧 transaction service 再减少约 `251` 行，无新增 migration。下一步隔离 `XiangyuanSettlementService`。

2026-09-26 base xiangyuan compatibility isolation：旧 `XiangyuanSettlementService`、`XiangyuanCreateResult` 与 `XiangyuanClaimResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_xiangyuan.py`；`xiangyuan.py` 稳定 facade 和旧 transaction import 继续 re-export/显式兼容，仙缘 create/claim/get_group/clear_all、attached `player_data.xiangyuan_limit`、历史 JSON 一次性导入、资产扣减/返还、operation replay/conflict 与跨库 rollback 语义保持不变。新增 `base.xiangyuan_service_isolated` 进度门禁，xiangyuan/stone robbery/contest、rename、lottery、progress 和 source 聚焦回归 `42 passed`，compileall、architecture、inventory、diff check 通过；旧 transaction service 再减少约 `496` 行，无新增 migration。下一步按 base transaction service 顺序处理突破与渡劫服务。

2026-09-26 base breakthrough compatibility isolation：旧 `BreakthroughService`、`DirectBreakthroughResult` 与 `ContinuousBreakthroughResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_breakthrough.py`；`breakthrough_tribulation.py`、`breakthrough_service.py`、base repository 和旧 transaction import 保持兼容，直接/连续突破及渡厄突破四类 operation ledger、状态快照、经验/属性更新、幂等冲突与 rollback 语义保持不变。新增 `base.breakthrough_service_isolated` 进度门禁，breakthrough/xiangyuan/stone robbery、progress 和 source 聚焦回归 `43 passed`，compileall、architecture、inventory、diff check 通过；旧 transaction service 再减少约 `796` 行，无新增 migration。下一步处理 `OrdinaryTribulationService`。

2026-09-26 base ordinary-tribulation compatibility isolation：旧 `OrdinaryTribulationService` 与 `OrdinaryTribulationResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_ordinary_tribulation.py`；普通渡劫 facade、突破兼容路径、base repository 和旧 transaction import 继续 re-export/显式兼容，成功/失败状态快照、天命丹扣除、统计写入、operation replay/conflict 与跨库 rollback 语义保持不变。新增 `base.ordinary_tribulation_service_isolated` 进度门禁；普通渡劫、突破、仙缘、抢灵石和来源质量聚焦回归 `48 passed`，`py_compile`、architecture、inventory、progress 与 `git diff --check` 通过；旧 transaction service 再减少约 `175` 行，无新增 migration。恢复演练和缓存清理完成后，下一步继续处理 `DestinyTribulationService`，全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成。

2026-09-27 base destiny-tribulation compatibility isolation：旧 `DestinyTribulationService` 与 `DestinyTribulationResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_destiny_tribulation.py`；天命渡劫 facade、真实突破 handler、base repository 和旧 transaction import 继续 re-export/显式兼容，天命丹扣除、等级/经验 expected snapshot、渡劫状态清理、统计写入、operation replay/conflict 与跨库 rollback 语义保持不变。新增 `base.destiny_tribulation_service_isolated` 进度门禁；天命/普通/突破渡劫及进度聚焦回归 `34 passed`，来源契约 `6 passed`，`py_compile`、architecture、inventory、progress 与 `git diff --check` 通过；旧 transaction service 再减少约 `89` 行，无新增 migration。五库 recovery 的 backup/restore、全量及 attached 迁移和 reconcile 均 clean；缓存与临时产物已清理。下一步继续处理 `HeartDevilTribulationService`，全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成。

2026-09-27 base heart-devil tribulation compatibility isolation：旧 `HeartDevilTribulationService` 与 `HeartDevilTribulationResult` 从 `xiuxian_base/transaction_service.py` 移至 `compatibility/legacy_base_heart_devil_tribulation.py`；心魔 facade、真实突破 handler 和旧 transaction import 继续 re-export/显式兼容，心魔计数/成功率、可选天命丹、战斗消息、expected snapshot、operation replay/conflict 与跨库 rollback 语义保持不变。新增 `base.heart_devil_tribulation_service_isolated` 进度门禁；心魔/天命/普通/突破渡劫及进度聚焦回归 `38 passed`，来源契约 `6 passed`，`py_compile`、architecture、inventory、progress 与 `git diff --check` 通过；旧 transaction service 再减少约 `148` 行，无新增 migration。五库 recovery 的 backup/restore、全量及 attached 迁移和 reconcile 均 clean；缓存与临时产物已清理。下一步继续处理 `PillFusionService`，全局 legacy transaction services 与 `xiuxian2_handle` blockers 仍未完成。
