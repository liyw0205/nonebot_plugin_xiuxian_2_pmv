...
2026-09-13 sign-in effects 边界：`SignInApplication` 新增 `SignInEffects` port 和默认 `NullSignInEffects`。资产事务提交后才调用 `on_signed(user_id, operation_id, stone, replayed)`；拒绝不会通知。新增 `tests/test_sign_in_effects_boundary.py` 覆盖提交/replay/拒绝；新增 `compatibility/sign_in_effects.py::LegacySignInEffects`，可在明确 compatibility wiring 下路由旧 lottery/statistics/task/log 副作用，保留 lottery 文案。`tests/test_legacy_sign_in_effects.py` 验证新 application 提交后路由旧副作用。当前发现同一 operation 可能经过 ledger replay 与旧 projection replay 两层，statistics/task 不能只靠进程内状态保证严格一次，必须增加持久化 side-effect event ledger；该 adapter 尚未接入默认 runtime，旧副作用仍不计入迁移完成。
