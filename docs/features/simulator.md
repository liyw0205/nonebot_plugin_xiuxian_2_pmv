...
## 数据模型与迁移
Migration `legacy.simulator.001` records the slice in `game_db`; the feature-owned repository and explicit service port now own the operation boundary while legacy tables remain the compatibility data source.
...
## 配置项
`simulator_enabled` controls the application boundary and defaults to true.
