...
`鬼市存灵石` 与 `鬼市取灵石` 默认通过 `TradeApplication` 的 feature-owned
use case 结算。操作记录在 game DB 的
`trade_guishi_deposit_operations`/`trade_guishi_withdraw_operations`，鬼市余额投影
保留在 trade DB 的 `guishi_info`；启动迁移 `trade.002`/`trade.003`/`trade.004`
先建立两端 schema。取出规则由注入 `Clock` 的 application 决定周末开放窗口，
并保留历史动态手续费公式。
## 用户流程
...
取出 operation 表，`trade.003` 只在 trade DB 创建或补齐 `guishi_info`。
## 事务与失败回滚
鬼市存取使用 game DB 主事务附加 trade DB，玩家钱包、余额投影和 operation
写入要么全部提交、要么全部回滚。两种操作均保留 canonical payload replay/conflict
和旧 `guishi_stone_operations` 回放兼容；订单和拍卖路径仍由兼容服务持有。
## 定时任务
...
## 灰度开关、回滚和已知限制
已完成的鬼市存取入口没有隐式回退；旧 `GuishiStoneService` 仅保留给尚未迁移的
交易兼容路径。拍卖策略仍由兼容服务提供。
