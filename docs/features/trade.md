...
`鬼市存灵石`、`鬼市取灵石` 和 `鬼市求购` 默认通过 `TradeApplication` 的
...
并保留历史动态手续费公式。
## 用户流程
鬼市存取、求购创建、上架/撤回、场次开始/收尾和现世购买统一提交操作号。
## 命令与别名
`交易`、`寄售`、`鬼市`。
## Web API
...
写入要么全部提交、要么全部回滚。三种操作均保留 canonical payload replay/conflict
...
## 测试与手工验收
覆盖求购重复请求、订单 ID 冲突、订单上限、余额不足、跨库失败和恢复重试。
## 灰度开关、回滚和已知限制
已完成的鬼市存取和求购创建入口没有隐式回退；旧 `GuishiStoneService` 仅保留给尚未
