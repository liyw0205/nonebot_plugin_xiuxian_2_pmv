...
from nonebot_plugin_xiuxian_2.xiuxian.xiuxian_rift.battle_reward_service import RiftBattleRewardService
from tests.test_db_backend import db_backend


class RiftBattleRewardServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.service = RiftBattleRewardService(self.database)
    ...
    def test_operation_insert_failure_rolls_back_every_member(self):
        with db_backend.transaction(self.database) as conn:
            conn.execute("CREATE TABLE rift_battle_reward_operations (operation_id TEXT PRIMARY KEY, payload TEXT NOT NULL, created_at TIMESTAMP)")
            conn.execute("CREATE TRIGGER fail_reward BEFORE INSERT ON rift_battle_reward_operations BEGIN SELECT RAISE(ABORT, 'fail'); END")
