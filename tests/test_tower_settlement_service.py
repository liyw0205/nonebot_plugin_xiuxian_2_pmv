...
class TowerSettlementServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with db_backend.transaction(self.game_database) as conn:
            conn.execute("CREATE TABLE user_xiuxian (user_id TEXT PRIMARY KEY, stone INTEGER, exp INTEGER, hp INTEGER, mp INTEGER, user_stamina INTEGER)")
            conn.execute("INSERT INTO user_xiuxian VALUES (%s, %s, %s, %s, %s, %s)", ("user", 10, 100, 50, 80, 12))
    ...
    def settle(self, operation_id="settlement", **overrides):
        ...
        extras = {key: values[key] for key in ("expected_player", "final_hp", "final_mp", "stamina_cost", "challenge_succeeded") if key in values}
        return self.service.settle(operation_id, "user", values["expected"], values["floor"], values["score"], values["stone"], values["exp"], values["items"], values["max_goods"], **extras)
    ...
    def state(self):
        ...
        with db_backend.connection(self.game_database) as conn:
            user = conn.execute("SELECT stone, exp, hp, mp, user_stamina FROM user_xiuxian WHERE user_id=%s", ("user",)).fetchone()
    ...
    def test_stale_state_and_full_inventory_leave_everything_unchanged(self) -> None:
        ...
        self.assertEqual(self.state(), ((9, 9, 50), (10, 100, 50, 80, 12), (99, 99)))

    def test_challenge_settlement_commits_vitals_stamina_and_rewards(self) -> None:
        result = self.settle(expected_player={"hp": 50, "mp": 80, "user_stamina": 12}, final_hp=17, final_mp=41, stamina_cost=3)
        self.assertEqual(result.status, "applied")
        self.assertEqual(self.state(), ((10, 10, 58), (30, 130, 17, 41, 9), (1, 1)))

    def test_failed_challenge_only_commits_vitals_and_stamina_idempotently(self) -> None:
        values = dict(expected_player={"hp": 50, "mp": 80, "user_stamina": 12}, final_hp=1, final_mp=33, stamina_cost=3, challenge_succeeded=False, score=0, stone=0, exp=0, items=[])
        first = self.settle("failed", **values)
        duplicate = self.settle("failed", **values)
        self.assertEqual((first.status, duplicate.status), ("applied", "duplicate"))
        self.assertEqual(self.state(), ((9, 9, 50), (10, 100, 1, 33, 9), None))
    ...
    def test_operation_write_failure_rolls_back_all_changes(self) -> None:
        ...
        self.assertEqual(self.state(), ((9, 9, 50), (10, 100, 50, 80, 12), None))
