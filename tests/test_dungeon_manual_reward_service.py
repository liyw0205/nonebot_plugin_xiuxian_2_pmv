...
from nonebot_plugin_xiuxian_2.xiuxian.xiuxian_dungeon.manual_reward_service import DungeonManualRewardService
from tests.test_db_backend import db_backend


class DungeonManualRewardServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.service = DungeonManualRewardService(self.database)
    ...
    def test_operation_insert_failure_rolls_back_every_member(self):
        with db_backend.transaction(self.database) as conn:
            conn.execute("CREATE TABLE dungeon_manual_reward_operations (operation_id TEXT PRIMARY KEY, payload TEXT NOT NULL, created_at TIMESTAMP)")
            conn.execute("CREATE TRIGGER fail_reward BEFORE INSERT ON dungeon_manual_reward_operations BEGIN SELECT RAISE(ABORT, 'fail'); END")
