...
class T(unittest.TestCase):
 ...
 def call(self,op='o',stone=100): return self.s.hatch(op,'u',stone,10,['',0,0,None],[(self.pet,True)],['x',1,0],10)
 def test_idempotent(self): self.assertEqual(self.call().status,'applied'); self.assertEqual(self.call().status,'duplicate')
 def test_snapshot(self): self.assertEqual(self.call(stone=99).status,'state_changed')
