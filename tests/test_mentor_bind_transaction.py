...
class MentorBindTests(unittest.TestCase):
    def setUp(self):
        ...
        with db_backend.transaction(self.player) as conn:
            conn.execute(
                ...
                "bind_time TEXT,breakthrough_reward_count INTEGER)"
            )
            conn.executemany(
                "INSERT INTO mentor VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                [
    ...
    def call(self, operation="op", max_apprentices=5, invite_id="invite-1", bind_time="2026-07-13 12:00:00"):
        return self.service.apply(
            ...
            max_apprentices=max_apprentices,
            history_limit=50,
            mentor_desc="收徒",
            apprentice_desc="拜师",
    ...
    def test_replays_legacy_payload_with_bind_time(self):
        ...
        with db_backend.transaction(self.game) as conn:
            payload = json.loads(
                conn.execute("SELECT payload FROM mentor_bind_operations WHERE operation_id='op'").fetchone()[0]
            )
    ...
    def test_invitation_capacity_cooldown_and_existing_mentor(self):
        ...
        with db_backend.transaction(self.player) as conn:
            conn.execute(
                "UPDATE mentor SET apprentice_ids='[]',mentor_cd_until='2099-01-01 00:00:00' WHERE user_id='m'"
            )
    ...
    def test_same_mentor_already_bound_is_idempotent_success(self):
        with db_backend.transaction(self.player) as conn:
            conn.execute(
                "UPDATE mentor SET mentor_id='m', bind_time='2026-07-01 00:00:00' WHERE user_id='a'"
            )
            conn.execute(
                "UPDATE mentor SET apprentice_ids=%s WHERE user_id='m'",
                (json.dumps(["a"]),),
            )
        apps = MentorApplicationService(self.player)
        apps.create("invite-same", "m", "a", now=100, ttl_seconds=1000)
        result = self.service.apply(
            "op-same",
            "m",
            "a",
            "invite-same",
            bind_time="2026-07-13 12:00:00",
            expected_mentor_level="洞虚境",
            expected_apprentice_level="筑基境",
            max_apprentices=5,
            history_limit=50,
            mentor_desc="收徒",
            apprentice_desc="拜师",
            now=datetime.fromtimestamp(200),
        )
        self.assertEqual("duplicate", result.status)
        self.assertTrue(result.succeeded)
        self.assertEqual("2026-07-01 00:00:00", result.bind_time)
    ...
    def test_failure_rolls_back_relation_statistics_and_history(self):
        ...
        with db_backend.transaction(self.game) as conn:
            ...
            conn.execute(
                "CREATE TRIGGER fail_bind BEFORE INSERT ON mentor_bind_operations "
                "BEGIN SELECT RAISE(ABORT,'fail'); END"
            )
        with db_backend.transaction(self.player) as conn:
            conn.execute("UPDATE mentor SET apprentice_ids='[]' WHERE user_id='m'")
            conn.execute(
                "UPDATE mentor SET mentor_id=NULL,bind_time=NULL,mentor_history='[]' WHERE user_id='a'"
            )
        ...
        with db_backend.connection(self.player) as conn:
            ...
            self.assertEqual(
                [],
                json.loads(conn.execute("SELECT apprentice_ids FROM mentor WHERE user_id='m'").fetchone()[0]),
            )
