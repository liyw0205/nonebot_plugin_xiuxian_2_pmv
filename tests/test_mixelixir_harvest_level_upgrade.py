...
class MixelixirHarvestLevelUpgradeTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with db_backend.transaction(self.player) as conn:
            conn.execute(
                ...
                f"{db_backend.quote_ident('收取等级')} TEXT,{db_backend.quote_ident('炼丹经验')} TEXT)"
            )
            conn.execute("INSERT INTO mix_elixir_info VALUES (%s,%s,%s)", ("user", "0", "900"))
        self.service = MixelixirHarvestLevelUpgradeService(self.game, self.player)
    ...
    def state(self):
        ...
        with db_backend.connection(self.player) as conn:
            mix = conn.execute(
                f"SELECT {db_backend.quote_ident('收取等级')},{db_backend.quote_ident('炼丹经验')} "
                "FROM mix_elixir_info WHERE user_id=%s",
                ("user",),
            ).fetchone()
        return stone, int(mix[0]), int(mix[1])

    def upgrade(self, operation_id="upgrade", level=0, experience=900, stone=5000, next_level=1, cost=150):
    ...
    def test_success_spends_experience_and_keeps_stones(self) -> None:
        result = self.upgrade()
        self.assertEqual((result.status, result.cost, result.wallet_stone, result.level, result.experience), ("applied", 150, 5000, 1, 750))
        self.assertEqual(self.state(), (5000, 1, 750))
    ...
    def test_duplicate_replays_result_and_conflicting_payload_is_rejected(self) -> None:
        ...
        conflict = self.upgrade("repeat", cost=151)
        self.assertEqual((first.status, duplicate.status, conflict.status), ("applied", "duplicate", "state_changed"))
        self.assertEqual((duplicate.cost, duplicate.wallet_stone, duplicate.level, duplicate.experience), (150, 5000, 1, 750))
        self.assertEqual(self.state(), (5000, 1, 750))
    ...
    def test_operation_write_failure_rolls_back_level_and_experience(self) -> None:
        with db_backend.transaction(self.game) as conn:
            conn.execute(
                "CREATE TABLE mixelixir_harvest_level_upgrade_operations ("
            ...
            conn.execute(
                "CREATE TRIGGER fail_harvest_upgrade BEFORE INSERT ON mixelixir_harvest_level_upgrade_operations "
