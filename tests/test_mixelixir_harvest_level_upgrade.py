...
class MixelixirHarvestLevelUpgradeTests(unittest.TestCase):
    ...
    def upgrade(self, operation_id="upgrade", level=0, experience=900, stone=5000, next_level=1, cost=1500):
    ...
    def test_success_charges_stones_and_upgrades_without_spending_experience(self) -> None:
        result = self.upgrade()
        self.assertEqual((result.status, result.cost, result.wallet_stone, result.level), ("applied", 1500, 3500, 1))
        self.assertEqual(self.state(), (3500, 1, 900))
    ...
    def test_duplicate_replays_result_and_conflicting_payload_is_rejected(self) -> None:
        ...
        conflict = self.upgrade("repeat", cost=1501)
        self.assertEqual((first.status, duplicate.status, conflict.status), ("applied", "duplicate", "state_changed"))
        self.assertEqual((duplicate.cost, duplicate.wallet_stone, duplicate.level), (1500, 3500, 1))
        self.assertEqual(self.state(), (3500, 1, 900))
