    ...
    def test_claim_replay_is_idempotent_across_all_effects(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with DatabaseUnitOfWork(database) as uow:
                ...
                winners = uow.query_one("SELECT COUNT(*) AS count FROM lottery_winner_history", ())
                economy = uow.query_one("SELECT COUNT(*) AS count FROM economy_log WHERE trace_id=?", ("lottery:sign-1",))
            ...
            assert user is not None and lottery is not None and task_events is not None and winners is not None and economy is not None
            ...
            self.assertEqual(int(winners["count"]), 1)
            self.assertEqual(int(economy["count"]), 1)
