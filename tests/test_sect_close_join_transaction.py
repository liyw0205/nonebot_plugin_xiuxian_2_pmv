...
class SectCloseJoinServiceTests(unittest.TestCase):
    def test_sect_facade_defers_close_join_service_construction(self):
        from nonebot_plugin_xiuxian_2.xiuxian import xiuxian_sect as sect_plugin

        self.assertIsNone(sect_plugin._sect_close_join_service_instance)
    ...
    def test_owner_closes_join_and_repeat_is_idempotent(self) -> None:
        first = self.service.close("close-1", "owner", expected_sect_id=1)
        second = self.service.close("close-1", "owner", expected_sect_id=1)

        self.assertEqual(first.status, "closed")
        self.assertEqual(second.status, "duplicate")
        self.assertEqual(self.value("SELECT join_open FROM sects WHERE sect_id=1"), 0)
        self.assertEqual(self.value("SELECT COUNT(*) FROM sect_close_join_operations"), 1)

    def test_close_rechecks_owner_closed_and_current_state(self) -> None:
        self.assertEqual(self.service.close("elder", "elder").status, "not_owner")
        with db_backend.transaction(self.database) as conn:
            conn.execute("UPDATE sects SET closed=1 WHERE sect_id=1")
        self.assertEqual(self.service.close("closed", "owner").status, "sect_closed")
        with db_backend.transaction(self.database) as conn:
            conn.execute("UPDATE sects SET closed=0, join_open=0 WHERE sect_id=1")
        self.assertEqual(self.service.close("already", "owner").status, "already_closed")

    def test_failure_rolls_back_state_and_operation(self) -> None:
        with db_backend.transaction(self.database) as conn:
            self.service._ensure_operations(conn)
            conn.execute(
                "CREATE TRIGGER fail_close_operation BEFORE INSERT "
                "ON sect_close_join_operations BEGIN "
                "SELECT RAISE(ABORT, 'operation failed'); END"
            )

        with self.assertRaises(db_backend.IntegrityError):
            self.service.close("fail", "owner")
