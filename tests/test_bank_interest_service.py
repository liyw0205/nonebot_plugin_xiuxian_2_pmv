...
from nonebot_plugin_xiuxian_2.xiuxian.xiuxian_bank.transaction_service import BankInterestService
...
class BankInterestServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.service = BankInterestService(self.game_database, self.player_database)
    ...
    def settle(self, operation_id="interest", expected_saved=500, interest=20, settled_at="new"):
        return self.service.settle(operation_id, "user", expected_saved, "old", "1", interest, settled_at)

    def test_success_credits_interest_and_advances_time(self) -> None:
        result = self.settle()
        self.assertEqual((result.status, result.interest, result.wallet_stone), ("applied", 20, 1020))
        self.assertEqual(self.state(), (1020, (500, "new", "1")))

    def test_zero_interest_still_advances_time(self) -> None:
        result = self.settle(interest=0)
        self.assertEqual((result.status, result.wallet_stone), ("applied", 1000))
        self.assertEqual(self.state(), (1000, (500, "new", "1")))

    def test_stale_account_state_changes_nothing(self) -> None:
        result = self.settle(expected_saved=400)
    ...
    def test_operation_failure_rolls_back_wallet_and_time(self) -> None:
        with db_backend.transaction(self.game_database) as conn:
            conn.execute(
                "CREATE TABLE bank_interest_operations (operation_id TEXT PRIMARY KEY, payload TEXT NOT NULL, "
                "interest INTEGER NOT NULL, wallet_stone INTEGER NOT NULL, saved_at TEXT NOT NULL, created_at TIMESTAMP)"
            )
            conn.execute(
                "CREATE TRIGGER fail_interest BEFORE INSERT ON bank_interest_operations "
