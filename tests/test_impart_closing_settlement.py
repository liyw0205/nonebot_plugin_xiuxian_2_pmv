...
import nonebot
nonebot.init()
from nonebot_plugin_xiuxian_2.xiuxian.xiuxian_impart_pk.transaction_service import ImpartClosingSettlementService
from tests.test_db_backend import db_backend


def test_impart_facade_defers_closing_settlement_service_construction():
    impart = importlib.import_module(
        "nonebot_plugin_xiuxian_2.xiuxian.xiuxian_impart_pk"
    )
    assert impart._impart_closing_settlement_service_instance is None


def test_impart_closing_handler_uses_lazy_three_database_service():
    source = Path(
    ...
    assert "_impart_closing_settlement_service().get_result(" in handler
    ...
    assert "impart_closing_settlement_service.settle(" not in handler
