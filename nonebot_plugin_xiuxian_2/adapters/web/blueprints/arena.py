...
def create_blueprint(*, application, permission) -> Blueprint:
    router = Blueprint("arena", __name__)
    ...
    def invoke(action: str):
        ...
        try:
            ...
            outcome = getattr(application, action)(operation_id=operation_id, **values)
    ...
    for path, action in (("purchase", "purchase"), ("challenge-purchase", "purchase_challenges"), ("settle", "settle")):
        endpoint = f"{action}_route"
        view = guard("user", permission, write=True)(lambda action=action: invoke(action))
        router.add_url_rule(f"/api/v1/arena/{path}", endpoint=endpoint, view_func=view, methods=["POST"])
    return router
