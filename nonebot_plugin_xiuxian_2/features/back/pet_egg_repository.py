...
class PetEggUseSqlRepository:
    ...
    def __init__(
        ...
        *,
        ...
        self._ensure_schema_on_run = bool(ensure_schema)
    ...
    def _current_pet_snapshot(conn, user_id: str) -> tuple[str, tuple[str, ...]] | None:
        meta = conn.execute(
            "SELECT active_uid FROM player_data.player_pet WHERE user_id=?",
            (user_id,),
        ).fetchone()
        if meta is None:
            return None
        rows = conn.execute(
            "SELECT uid FROM player_data.player_pet_item WHERE user_id=? ORDER BY uid",
            (user_id,),
        ).fetchall()
    ...
    def use_pet_eggs(
        ...
        *,
        ...
        with self._lock, AttachedDatabaseUnitOfWork(
            self._game_database,
            attachments={"player_data": self._player_database},
            immediate=True,
        ) as conn:
            try:
                if self._ensure_schema_on_run:
                    self._ensure_schema(conn)
                previous = conn.execute(
                    "SELECT user_id, item_id, quantity, pets_json "
                    "FROM batch_pet_egg_use_operations WHERE operation_id=?",
                    (operation_id,),
                ).fetchone()
                ...
                columns = {
                    str(row["name"])
                    for row in conn.execute("PRAGMA table_info(back)").fetchall()
                }
                bind_update = ""
                params: list[object] = [quantity]
                if "bind_num" in columns:
                    bind_update = ", bind_num=MIN(COALESCE(bind_num, 0), goods_num-?)"
                    params.append(quantity)
                consumed = conn.execute(
                    f"UPDATE back SET goods_num=goods_num-?{bind_update} "
                ...
                conn.execute(
                    "UPDATE player_data.player_pet SET active_uid=? WHERE user_id=?",
                    (active_uid, user_id),
                )
                pets_json = json.dumps(normalized_pets, ensure_ascii=False)
                conn.execute(
                    "INSERT INTO batch_pet_egg_use_operations "
                    "(operation_id,user_id,item_id,quantity,pets_json) VALUES (?,?,?,?,?)",
...
__all__ = ["BatchPetEggUseResult", "PetEggUseSqlRepository"]
