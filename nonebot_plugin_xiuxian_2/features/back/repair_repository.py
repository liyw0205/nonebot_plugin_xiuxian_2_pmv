...
class _RepairConnection:
    """Small adapter preserving the legacy repair query helpers on the shared UoW."""

    def __init__(self, database: str | Path) -> None:
        self._uow = DatabaseUnitOfWork(database, immediate=True)
    ...
    def __exit__(self, exc_type: Any, exc: Any, traceback: Any) -> bool | None:
        return self._uow.__exit__(exc_type, exc, traceback)

    def execute(self, sql: str, params: Any = ()):
        return self._uow.execute(sql.replace("%s", "?"), params)

    def commit(self) -> None:
        self._uow.commit()

    def rollback(self) -> None:
        if self._uow.connection is not None:
            self._uow.connection.rollback()
...
class BackpackRepairSqlRepository:
    ...
    def run(
        ...
        *,
        ...
        with self._lock, _RepairConnection(self._database) as conn:
            try:
...
__all__ = ["BackpackRepairResult", "BackpackRepairSqlRepository"]
