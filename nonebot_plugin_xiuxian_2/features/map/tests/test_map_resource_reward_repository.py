...
class MapResourceRewardRepositoryTests(unittest.TestCase):
    ...
    def test_applies_assets_and_completes_action(self):
        ...
        with db_backend.transaction(self.game) as conn:
            self.assertEqual((12, 2), tuple(conn.execute("SELECT stone,goods_num FROM user_xiuxian JOIN back USING(user_id)").fetchone()))
        with db_backend.transaction(self.player) as conn:
            self.assertEqual(("completed", "2026-09-14 00:02:00"), tuple(conn.execute("SELECT status,gather_cd_until FROM map_interactive_actions JOIN map_cooldown USING(user_id)").fetchone()))
