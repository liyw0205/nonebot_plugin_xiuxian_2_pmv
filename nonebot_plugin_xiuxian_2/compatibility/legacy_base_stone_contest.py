...
class StoneContestService:
    """Legacy stone contest and theft settlement kept as an explicit rollback path."""
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            try:
                ...
                previous = conn.execute(
                    ...
                    "operation_type FROM stone_contest_operations WHERE operation_id=%s", (operation_id,),
                ).fetchone()
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            try:
                ...
                if thief_stamina < stamina_cost:
                    conn.rollback()
                    return current_result("stamina_insufficient", thief_stamina=thief_stamina)
                if thief_stone < penalty_amount:
                    conn.rollback()
                    return current_result("stone_insufficient", thief_stamina=thief_stamina)
                if victim_stone <= 0:
                    conn.rollback()
