"""Explicit rollback implementation for the pre-application sign-in path."""
...
class SignInService:
    """Old synchronous contract retained only for explicit rollback/tests."""

    def __init__(self, database: str | Path, lock: RLock | None = None, randint: Callable[[int, int], int] | None = None) -> None:
    ...
    def _ensure_operations(conn) -> None:
        conn.execute(
            ...
            "operation_id TEXT PRIMARY KEY,user_id TEXT NOT NULL,stone INTEGER NOT NULL,"
    ...
    def get_result(self, operation_id: str) -> SignInResult | None:
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            ...
            previous = conn.execute(
                "SELECT user_id, stone FROM sign_in_operations WHERE operation_id=%s", (operation_id,)
            ).fetchone()
            if previous is None:
                return None
    ...
    def sign(self, operation_id, user_id, stone_lower: int, stone_upper: int) -> SignInResult:
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            try:
                ...
                self._ensure_operations(conn)
                previous = conn.execute(
                    "SELECT stone FROM sign_in_operations WHERE operation_id=%s", (operation_id,)
                ).fetchone()
                if previous:
                    conn.rollback()
                    return SignInResult("duplicate", user_id, int(previous[0]))
                users = conn.execute(
                    "SELECT CAST(COALESCE(is_sign,0) AS INTEGER) FROM user_xiuxian WHERE user_id=%s", (user_id,)
                ).fetchall()
                if not users:
                    conn.rollback()
                ...
                updated = conn.execute(
                    "UPDATE user_xiuxian SET is_sign=1, stone=CAST(COALESCE(stone,0) AS REAL)+CAST(%s AS REAL) "
                ...
                if updated.rowcount < 1:
                    ...
                    return SignInResult("already_signed", user_id)
                conn.execute(
                    "INSERT INTO sign_in_operations(operation_id,user_id,stone) VALUES (%s,%s,%s)",
