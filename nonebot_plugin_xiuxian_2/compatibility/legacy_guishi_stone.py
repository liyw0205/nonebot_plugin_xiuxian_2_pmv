"""Explicit rollback service for the pre-feature Guishi stone path."""
...
from ..xiuxian.xiuxian_utils.numeric_bind import as_int_like
...
class LegacyGuishiStoneService:
    """Old synchronous implementation, retained only for explicit rollback."""
    ...
    def _execute(self, operation_id, operation_type, user_id, amount) -> GuishiStoneResult:
        ...
        if amount > self.OP_AMOUNT_CAP:
            return GuishiStoneResult("amount_capped", operation_type, user_id, amount)
            ...
            try:
                ...
                if previous is not None:
                    ...
                    if str(previous[0]) != operation_type or str(previous[1]) != user_id:
                        return GuishiStoneResult("state_changed", operation_type, user_id, amount)
                ...
                if stored_balance > self.STORED_CAP:
                    ...
                    conn.execute(
                        ...
                        "DO UPDATE SET stored_stone=EXCLUDED.stored_stone",
                ...
                if operation_type == "deposit":
                    if stored_balance + amount > self.STORED_CAP:
                        conn.rollback()
                        return GuishiStoneResult(
                            "stored_cap_exceeded", operation_type, user_id, amount,
                    ...
                    new_balance, fee, actual_amount = stored_balance + amount, 0, amount
                else:
                ...
                conn.execute(
                    ...
                    "(operation_id, operation_type, user_id, amount, fee, actual_amount, stored_balance) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    (operation_id, operation_type, user_id, amount, fee, actual_amount, new_balance),
                )
                conn.commit()
                return GuishiStoneResult(
                    "completed", operation_type, user_id, amount, fee, actual_amount, new_balance
                )
            except Exception:
                conn.rollback()
                raise
...
__all__ = ["GuishiStoneResult", "LegacyGuishiStoneService"]
