...
from ..xiuxian.xiuxian_buff.relation_transaction_utils import increment_stat
...
class HeartDevilTribulationService:
    ...
    def settle(
        ...
        *,
        ...
        operation_id, user_id = str(operation_id).strip(), str(user_id)
        expected_rate, expected_count, new_rate = map(
            int, (expected_rate, expected_count, new_rate)
        )
        ...
        with self._lock, closing(db_backend.connect(self._game_database)) as conn:
            ...
            try:
                ...
                conn.execute(
                    ...
                    "successful INTEGER NOT NULL,rate INTEGER NOT NULL,"
                    "heart_devil_count INTEGER NOT NULL,item_used INTEGER NOT NULL,"
                ...
                if previous:
                    ...
                    if saved.user_id != user_id:
                        return HeartDevilTribulationResult(
                            "operation_conflict", False, 0, 0, False
                        )
                    return saved
                state = conn.execute(
                    "SELECT current_rate,heart_devil_count "
                    "FROM user_tribulation WHERE user_id=%s",
                    (user_id,),
                ).fetchone()
                actual_rate, actual_count = (
                    (int(state[0]), int(state[1])) if state else (30, 0)
                )
                if actual_rate != expected_rate or actual_count != expected_count:
                    conn.rollback()
                    return HeartDevilTribulationResult(
                        "state_changed", False, actual_rate, actual_count, False
                    )
                if consume_destiny_pill:
                    consumed = conn.execute(
                        "UPDATE back SET goods_num=goods_num-1,"
                        "bind_num=MIN(COALESCE(bind_num,0),goods_num-1),"
                        "day_num=COALESCE(day_num,0)+1,all_num=COALESCE(all_num,0)+1,"
                        "update_time=%s,action_time=%s "
                    ...
                    if consumed.rowcount != 1:
                        conn.rollback()
                        return HeartDevilTribulationResult(
                            "item_missing", False, actual_rate, actual_count, False
                        )
                new_count = expected_count + 1
                changed = conn.execute(
                    "UPDATE user_tribulation SET current_rate=%s,"
                    "heart_devil_count=%s,last_time=%s "
                ...
                if changed.rowcount == 0 and state is None:
                    conn.execute(
                        ...
                        "(user_id,current_rate,heart_devil_count,last_time) "
                        "VALUES(%s,%s,%s,%s)",
                ...
                conn.execute(
                    "INSERT INTO heart_devil_tribulation_operations "
                    "(operation_id,payload,successful,rate,heart_devil_count,item_used) "
                    "VALUES(%s,%s,%s,%s,%s,%s)",
