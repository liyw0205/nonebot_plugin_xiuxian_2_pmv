"""Explicit rollback implementation for the pre-application stone gift path.

This module is intentionally outside the normal feature/application path. It is
loaded only when ``XIUXIAN_STONE_GIFT_LEGACY_HANDLER`` or the compatibility
facade explicitly requests the old implementation.
"""
...
class StoneGiftService:
    """Old synchronous contract retained only for explicit rollback."""
    ...
    def transfer(
        ...
        *,
        ...
        def result(status: str, gross=gross_amount, net=net_amount, fee=fee_amount):
            return StoneGiftResult(status, sender_id, recipient_id, int(gross), int(net), int(fee))
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            try:
                ...
                recipient = conn.execute(
                    "SELECT 1 FROM user_xiuxian WHERE user_id=%s", (recipient_id,)
                ).fetchone()
                ...
                conn.execute(
                    ...
                    "VALUES (%s, %s, %s, %s, %s, %s)",
                    (operation_id, sender_id, recipient_id, gross_amount, net_amount, fee_amount),
                )
                conn.commit()
    ...
    def get_result(self, operation_id: str) -> StoneGiftResult | None:
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            ...
            if previous is None:
                return None
            return StoneGiftResult("duplicate", str(previous[0]), str(previous[1]), int(previous[2]), int(previous[3]), int(previous[4]))

    def get_operation(self, operation_id, sender_id, recipient_id) -> StoneGiftResult | None:
        operation_id = str(operation_id).strip()
        sender_id = str(sender_id)
        recipient_id = str(recipient_id)
        if not operation_id:
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            ...
            if str(previous_sender) != sender_id or str(previous_recipient) != recipient_id:
                return None
            return StoneGiftResult("duplicate", sender_id, recipient_id, int(gross), int(net), int(fee))
