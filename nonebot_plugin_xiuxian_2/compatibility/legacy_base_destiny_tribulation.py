    ...
    def settle(
        ...
        *,
        ...
        if not operation_id:
            raise ValueError("operation_id must not be empty")
        payload = json.dumps(
            [user_id, expected_level, expected_exp, target_level, power, occurred_at],
            separators=(",", ":"),
        )
        with self._lock, closing(db_backend.connect(self._game_database)) as conn:
            ...
            try:
                ...
                conn.execute(
                    ...
                    "operation_id TEXT PRIMARY KEY,payload TEXT NOT NULL,target_level TEXT NOT NULL,"
                ...
                if (
                    user is None
                    or str(user[0]) != expected_level
                    or int(user[1] or 0) != expected_exp
                ):
                    conn.rollback()
                    return DestinyTribulationResult("state_changed", target_level, user_id)
                consumed = conn.execute(
                    ...
                    "WHERE user_id=%s AND goods_id=1997 AND goods_num>0",
                    (occurred_at, occurred_at, user_id),
                )
                if consumed.rowcount != 1:
                    conn.rollback()
                    return DestinyTribulationResult("item_missing", target_level, user_id)
                changed = conn.execute(
                    "UPDATE user_xiuxian SET level=%s,power=%s "
                    "WHERE user_id=%s AND level=%s AND exp=%s",
                ...
                conn.execute(
                    "INSERT INTO destiny_tribulation_operations "
                    "(operation_id,payload,target_level) VALUES(%s,%s,%s)",
                    (operation_id, payload, target_level),
                )
                conn.commit()
                return DestinyTribulationResult("applied", target_level, user_id)
            except Exception:
                conn.rollback()
                raise
            finally:
