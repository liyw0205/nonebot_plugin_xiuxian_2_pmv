...
from .features.map.migrations import apply_map, apply_map_explore_player, apply_map_explore_settlement, apply_map_explore_start, apply_map_home_return, apply_map_interactive_player, apply_map_interactive_start, apply_map_mission_claim, apply_map_movement, apply_map_resource_reward, apply_map_seed_purchase
...
def build_lifecycle(context: RuntimeContext | None = None) -> tuple[Lifecycle, Readiness, RuntimeContext]:
    ...
    game_migrations = tuple(
        ...
        if migration.version not in {"combat_settlement.003", "combat_settlement.004", "map.003", "map.005", "map.008", "tianti_settlement.002", "tianti_training.003", "tianti_training.004", "tianti_training.005", "tianti_training.006"}
    )
            ...
            with DatabaseUnitOfWork(spec.path) as uow:
                if spec.key == "game_db":
                    ...
                    runner = MigrationRunner(
                        tuple(migration for migration in migration_runner.migrations if migration.version in {"title.001", "combat_settlement.003", "combat_settlement.004", "map.003", "map.005", "map.008", "tianti_settlement.002", "tianti_training.003", "tianti_training.004", "tianti_training.005"}),
                        clock=context.clock,
                    )
                else:
