...
def build_lifecycle(context: RuntimeContext | None = None) -> tuple[Lifecycle, Readiness, RuntimeContext]:
    ...
    game_migrations = tuple(
        ...
        if migration.version not in {"combat_settlement.003", "combat_settlement.004", "map.003", "map.005", "map.008", "map.013", "tianti_settlement.002", "tianti_training.003", "tianti_training.004", "tianti_training.005", "tianti_training.006"}
    )
            ...
            with DatabaseUnitOfWork(spec.path) as uow:
                if spec.key == "game_db":
                    ...
                    runner = MigrationRunner(
                        tuple(migration for migration in migration_runner.migrations if migration.version in {"title.001", "combat_settlement.003", "combat_settlement.004", "map.003", "map.005", "map.008", "map.013", "tianti_settlement.002", "tianti_training.003", "tianti_training.004", "tianti_training.005"}),
                        clock=context.clock,
                    )
                else:
