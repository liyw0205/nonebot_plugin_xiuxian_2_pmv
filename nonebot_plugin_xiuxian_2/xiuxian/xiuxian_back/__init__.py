...
def _unbind_item_service():
    ...
    if _unbind_item_service_instance is None:
        _unbind_item_service_instance = UnbindItemService(get_paths().game_db)
    return _unbind_item_service_instance

def _lottery_talisman_service():
    global _lottery_talisman_service_instance
...
def configure_back_application(application: BackApplication) -> None:
    ...
    _accessory.configure_decompose_application(application.accessory_decompose_application)
...
async def use_(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if goods_type == "礼包":
        ...
        if accessory_need > 0:
            ...
            for rewards in package_rewards:
                for rwd in rewards:
                    ...
                    if r_buff is None:
                        ...
                        continue
                    if r_type == "饰品":
                        quality = max(1, min(5, int(rwd.get("quality", 1) or 1)))
                        for _ in range(r_amount):
                            accessory_instances.append(
                                create_accessory_instance(int(r_buff), quality)
                            )
                        continue
                    goods_reward_type = (
            ...
            if validation_errors or len(accessory_instances) != accessory_need:
...
async def use_pet_egg_item(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, item_id: int, num: int):
    ...
    result = back_application.use_pet_eggs(
