...
danyao_back = on_command('丹药背包', priority=10, permission=GROUP, block=True)
...
# 仙肆查看
xian_shop = on_command("仙肆查看", aliases={"查看仙肆"}, priority=8, permission=GROUP, block=True)
# 仙肆上架（仅限超管）
xian_shop_added_by_admin = on_command("系统仙肆上架", priority=5, permission=SUPERUSER, block=True)
# 仙肆上架（用户权限）
xian_shop_add = on_command("仙肆上架", priority=5, permission=GROUP, block=True)
# 仙肆购买
...
__back_help__ = f"""
指令：
1、我的背包、药材背包、丹药背包:查看自身背包前196个物品的信息
2、使用+物品名字：使用物品,可批量使用
3、换装+装备名字：卸载目标装备
4、坊市购买+物品编号:购买坊市内的物品，可批量购买
5、坊市查看、查看坊市:查询坊市在售物品
6、仙肆查看、查看仙肆:查询全服仙肆在售物品
7、仙肆购买+物品编号:购买仙肆内的物品，可批量购买
8、仙肆上架:仙肆上架 物品 金额 [数量]，上架背包内的物品到全服仙肆，最低金额50w，可批量上架
9、查看拍卖品、拍卖品查看:查询将在拍卖品拍卖的玩家物品
10、坊市上架:坊市上架 物品 金额，上架背包内的物品,最低金额50w，可批量上架
11、提交拍卖品:提交拍卖品 物品 金额，上架背包内的物品,最低金额随意，可批量上架(需要超管重启机器人)
12、系统坊市上架:系统坊市上架 物品 金额，上架任意存在的物品，超管权限
...
@my_xian_shop.handle(parameterless=[Cooldown(1.4, at_sender=False, isolate_level=CooldownIsolateLevel.GROUP)])
async def my_xian_shop_(bot: Bot, event: GroupMessageEvent):
    """查看用户在全服仙肆上架的物品"""
    send_group_id = event.group_id  # 获取当前群号
    isUser, user_info, msg = check_user(event)
    if not isUser:
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=send_group_id, message=MessageSegment.image(pic))
        else:
            await bot.send_group_msg(group_id=send_group_id, message=msg)
        await my_xian_shop.finish()

    user_id = user_info["user_id"]
    shop_data = get_shop_data("000000")  # 使用全服仙肆 group_id
    user_items = []

    for item_id, item_info in shop_data.get("000000", {}).items():
        if str(item_info["user_id"]) == str(user_id):
            msg = f"编号：{item_id}\n"
            msg += f"{item_info['desc']}\n"
            msg += f"价格：{item_info['price']} 灵石\n"
            msg += f"数量：{item_info['stock']}\n"
            user_items.append(msg)
...
async def xian_shop_add_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    """仙肆上架"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    isUser, user_info, msg = check_user(event)
    if not isUser:
    ...
    try:
        ...
        if price < 500000:  # 设置最低上架金额，例如 50 万灵石
            raise ValueError("上架金额不得低于 50 万灵石！")
        if quantity <= 0:
            raise ValueError("上架数量必须为正数！")
    except ValueError as e:
        msg = str(e) if str(e).startswith("上架金额") or str(e).startswith("上架数量") else "请输入有效的金额和数量！"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
    ...
    goods_id = None
    for item in back:
        if item[1] == goods_name:
            goods_id = item[0]
            break
    if goods_id is None:
        msg = f"背包中不存在物品：{goods_name}！"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
    ...
    # 检查物品数量
    user_goods = sql_message.get_back_msg(user_id, goods_id=goods_id)
    if not user_goods or user_goods[0][2] < quantity:
        msg = f"背包中 {goods_name} 数量不足，无法上架 {quantity} 个！"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
    ...
    save_shop(shop_data)
    sql_message.update_back_j(user_id, goods_id, quantity, 2)  # 从背包移除物品

    msg = f"物品 {goods_name} 已成功上架仙肆，金额：{price} 灵石，数量：{quantity}！"
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
    else:
...
# 仙肆下架（用户和超管权限）
xian_shop_remove = on_command("仙肆下架", priority=5, permission=GROUP, block=True)

@xian_shop_remove.handle(parameterless=[Cooldown(1.4, at_sender=False, isolate_level=CooldownIsolateLevel.GROUP, parallel=1)])
async def xian_shop_remove_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    """仙肆下架"""
    ...
    try:
        ...
        if goods_owner_id != 0 and str(goods_owner_id) == str(user_id):
            ...
            msg = f"已成功下架仙肆中的物品：{goods_name}（编号：{item_id}），{goods_quantity} 个已退回你的背包！"
        else:
            msg = f"已成功下架仙肆中的物品：{goods_name}（编号：{item_id}）！"
...
@xian_shop.handle(parameterless=[Cooldown(at_sender=False)])
async def xian_shop_(bot: Bot, event: GroupMessageEvent):
    """仙肆查看"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    isUser, user_info, msg = check_user(event)
    if not isUser:
...
async def xian_shop_added_by_admin_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    """系统上架仙肆"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
...
async def xian_buy_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    """仙肆购买"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    async with buy_lock:
        isUser, user_info, msg = check_user(event)
...
async def yaocai_back_(bot: Bot, event: GroupMessageEvent):
    ...
    if len(msg) >= 98: #背包更新
        # 将第一条消息和第二条消息合并为一条消息
        msg1 = [f"{user_info['user_name']}的背包"] + msg[:98]
        msg2 = [f"{user_info['user_name']}的背包"] + msg[98:]
        try:
            await send_msg_handler(bot, event, '背包', bot.self_id, msg1)
            if msg2:
                await send_msg_handler(bot, event, '背包', bot.self_id, msg2)
...
async def danyao_back_(bot: Bot, event: GroupMessageEvent):
    ...
    if len(msg) >= 98: #背包更新
        ...
        try:
            ...
            await danyao_back.finish("查看背包失败!", reply_message=True)
    else:
        msg = msg
        try:
            await send_msg_handler(bot, event, '背包', bot.self_id, msg)
