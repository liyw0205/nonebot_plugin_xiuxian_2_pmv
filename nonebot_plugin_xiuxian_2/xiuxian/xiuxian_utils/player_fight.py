    ...
    while True:
        ...
        if player1_skil_open:  # 是否开启技能
                ...
                if player1_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(player1, user1_hp_cost, user1_mp_cost, player1_turn_cost,
                        ...
                        if user1_skill_type == 1:  # 直接伤害类技能
                            ...
                            play_list.append(
                                get_msg_dict(player1, player1_init_hp, f"{player2['道号']}剩余血量{number_to2(player2['气血'])}"))
                            ...
                            if user1_buff_type == 1:  # 攻击类buff
                                ...
                                play_list.append(
                                    get_msg_dict(player1, player1_init_hp,
                                                 f"{player2['道号']}剩余血量{number_to2(player2['气血'])}"))
                    ...
                    if user1_skill_type == 2:  # 持续性伤害技能
                        ...
                        player1_turn_cost = player1_turn_cost - 1
                        player1_sh = player1_sh * (user1_skill_data['turncost'] - player1_turn_cost)
                        play_list.append(get_msg_dict(player1, player1_init_hp,
                                                      f"{user1_skill_data['name']}叠伤剩余:{player1_turn_cost}回合"))
                        play_list.append(
                            get_msg_dict(player1, player1_init_hp, msg1.format(player1['道号'], number_to2(player1_sh))))
                        player2['气血'] = player2['气血'] - int(player1_sh * player2_js)  # 玩家1的伤害 * 玩家2的减伤
                        play_list.append(
            ...
            if user1_turn_skip:
                ...
                play_list.append(get_msg_dict(player1, player1_init_hp, f"{player2['道号']}剩余血量{number_to2(player2['气血'])}"))
        ...
        if player2_skil_open:  # 有技能
            if user2_turn_skip:  # 玩家2无需跳过回合
                ...
                if player2_turn_cost == 0:  # 没有持续性技能生效
                    ...
                    if isEnableUserSikll(player2, user2_hp_cost, user2_mp_cost, player2_turn_cost,
                                         skill_rate):  # 满足技能要求，#此处为技能的第一次释放
                        skill_msg, user2_skill_sh, player2_turn_cost = get_skill_sh_data(player2, user2_skill_data)
                        if user2_skill_type == 1:  # 直接伤害类技能
                            ...
                            play_list.append(
                                get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{number_to2(player1['气血'])}"))
                            ...
                            user2_buff_type = user2_skill_data['bufftype']
                            if user2_buff_type == 1:  # 攻击类buff
                                ...
                                play_list.append(
                                    get_msg_dict(player2, player2_init_hp,
                                                 f"{player1['道号']}剩余血量{number_to2(player1['气血'])}"))
                                player2 = calculate_skill_cost(player2, user2_hp_cost, user2_mp_cost)
                    ...
                    if user2_skill_type == 2:  # 持续性伤害技能
                        ...
                        skill_msg = get_persistent_skill_msg(player2['道号'], user2_skill_data['name'], user2_skill_sh,
                                                             player2_turn_cost)
                        play_list.append(get_msg_dict(player2, player2_init_hp, skill_msg))
                        ...
                        if user2_buff_type == 1:  # 攻击类buff
                            ...
                            play_list.append(get_msg_dict(player2, player2_init_hp,
                                                          f"{user2_skill_data['name']}增伤剩余{player2_turn_cost}回合"))
                            play_list.append(
                                get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], number_to2(player2_sh))))
                            player1['气血'] = player1['气血'] - int(player2_sh * player1_js)  # 玩家2的伤害 * 玩家1的减伤
                            play_list.append(
                            ...
                            player2_turn_cost = player2_turn_cost - 1
                            play_list.append(get_msg_dict(player2, player2_init_hp,
                                                          f"{user2_skill_data['name']}减伤剩余{player2_turn_cost}回合！"))
                            play_list.append(
                                get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], number_to2(player2_sh))))
                        ...
                        player2_turn_cost = player2_turn_cost - 1
                        player2_sh = player2_sh * (user2_skill_data['turncost'] - player2_turn_cost)
                        play_list.append(get_msg_dict(player2, player2_init_hp,
                                                      f"{user2_skill_data['name']}叠伤剩余:{player2_turn_cost}回合"))
                        play_list.append(
                            get_msg_dict(player2, player2_init_hp, msg2.format(player2['道号'], number_to2(player2_sh))))
                        player1['气血'] = player1['气血'] - int(player2_sh * player1_js)  # 玩家2的伤害 * 玩家1的减伤
                        play_list.append(
            ...
            if user2_turn_skip:
                ...
                play_list.append(get_msg_dict(player2, player2_init_hp, f"{player1['道号']}剩余血量{number_to2(player1['气血'])}"))
    ...
    while True:
        ...
        if player1_skil_open:  # 是否开启技能
            if user1_turn_skip:  # 无需跳过回合
                ...
                user1hpconst, user1mpcost, user1_skill_type, skillrate = get_skill_hp_mp_data(player1, user1_skill_data)
                if player1_turn_cost == 0:  # 没有持续性技能生效
                    player1_js = player1_f_js  # 没有持续性技能生效,减伤恢复
                    if isEnableUserSikll(player1, user1hpconst, user1mpcost, player1_turn_cost,
                                         skillrate):  # 满足技能要求，#此处为技能的第一次释放
                        skillmsg, user1_skill_sh, player1_turn_cost = get_skill_sh_data(player1, user1_skill_data)
                        if user1_skill_type == 1:  # 直接伤害类技能
                            ...
                            boss_hp_msg = f"{boss['name']}剩余血量{number_to2(boss['气血'])}"
                            ...
                            user1buff_type = user1_skill_data['bufftype']
                    ...
                    if user1_skill_type == 2:  # 持续性伤害技能
                        player1_turn_cost = player1_turn_cost - 1
                        skillmsg = get_persistent_skill_msg(player1['道号'], user1_skill_data['name'], user1_skill_sh,
                                                            player1_turn_cost)
                        play_list.append(get_msg_dict(player1, player_init_hp, skillmsg))
                        isCrit, player1_sh = get_turnatk(player1, 0, user1_battle_buff_date,
                                                         boss_buff, random_buff)  # 判定是否暴击 辅修功法14
            ...
            if boss_sub <= 6:
                ...
                player1['气血'] = player1['气血'] - ((
                            (boss_sh * (1 + boss_buff.boss_zs) * (player1_js - random_buff.random_def + boss_cj) * 5) + (
                                player1['气血'] * 0.3)))
                play_list.append(get_boss_dict(boss, qx, f"{player1['道号']}剩余血量{number_to2(player1['气血'])}", bot_id))
...
def get_persistent_skill_msg(username, skillname, sh, turn):
    ...
    return f"{username}的持续性技能：{skillname}，造成{number_to2(sh)}伤害，剩余回合：{turn}!"
...
def after_atk_sub_buff_handle(player1_sub_open, player1, user1_main_buff_data, subbuffdata1, damage1, player2,
    ...
    if buff_type == '4':
        ...
        msg = "回复气血:" + str(number_to2(restore_health))
    elif buff_type == '5':
        ...
        msg = None
        if mana_stolen > 0:
            msg = "吸取真元:" + str(number_to2(mana_stolen))
    elif buff_type == '8':
        ...
        msg = None
        if poison_damage > 0:
            msg = "对手中毒消耗血量:" + str(number_to2(poison_damage))
    elif buff_type == '9':
        ...
        if health_stolen > 0 or mana_stolen > 0:
            msg = f"吸取气血: {str(number_to2(health_stolen))}, 吸取真元: {str(number_to2(mana_stolen))}"
