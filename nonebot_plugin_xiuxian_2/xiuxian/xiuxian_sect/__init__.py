...
> 浏览全服宗门
- 创建宗门
> 消耗{XiuConfig().sect_create_cost}灵石（需境界{XiuConfig().sect_min_level}）
- 加入宗门 [ID/名称]
> 申请加入指定宗门
- 退出宗门
> 离开当前宗门
...
**建设**
- 宗门捐献
> 提升建设度（每{config["等级建设度"]}建设度提升1级修炼上限）
- 升级攻击修炼
> 每级约+4%攻击
- 升级元血修炼
> 每级约+8%气血
...
**任务与商店**
- 宗门任务接取
> 获取任务（每日上限：{config["每日宗门任务次上限"]}次）
- 宗门任务完成
> 提交任务（CD：{config["宗门任务完成cd"]}秒）
- 宗门任务刷新
> 更换任务（CD：{config["宗门任务刷新cd"]}秒）
- 宗门周常
> 查看全宗本周协作目标
- 领取宗门周常
> 领取已完成的周常奖励
...
> 外门弟子无法获得修炼资源；建设度影响宗门实力。管理指令请看【宗门管理】。
""".strip()
...
> 长老及以上可变更职位、踢出低于自己的成员。
> 宗主可开放/关闭加入、改名、传位、封闭或解散宗门。
> 长期不活跃宗主会降职，长期不活跃宗门可能自动解散。
""".strip()
        ...
        if sect_position == 15:
            ...
            if not result.succeeded:
                failure_messages = {
                    ...
                    "membership_changed": "宗门成员身份已变（已退出/被移出/职位变更），请重新查看宗门信息。",
    ...
    if give_qq:
        if give_qq == user_id:
            ...
            if result.succeeded:
                ...
                msg = "传位未完成：宗主身份或目标成员信息已变，请刷新宗门信息后重试。"
                await handle_send(bot, event, msg)
                await sect_owner_change.finish()
    else:
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if not result.succeeded:
        failure_messages = {
            ...
            "state_changed": "兑换时贡献/库存被改动，未结算，请重试。",
        }
        await handle_send(
            bot,
            event,
