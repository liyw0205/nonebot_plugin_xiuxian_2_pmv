...
def claim_collect_phrase(user_id: str, query: str, operation_id: str | None = None) -> tuple[bool, str]:
    ...
    if not result.succeeded:
        ...
        return False, {"limit_reached":"该词组已达到兑换次数上限","inventory_full":"背包空间不足，奖励未领取","user_missing":"角色不存在","operation_conflict":"兑换请求冲突，请重新发送"}.get(result.status,"兑换未结算：数据刚被其他操作改动，请重试")
...
def claim_activity_tasks(user_id: str, query: str = "", operation_id: str | None = None) -> tuple[bool, str]:
    ...
    if not result.succeeded:
        messages = {
            ...
            "user_missing": "角色不存在",
            "state_changed": "任务领取未结算：任务数据刚被改动，请重新查询",
...
def claim_activity_pass_rewards(user_id: str, query: str = "", operation_id: str | None = None) -> tuple[bool, str]:
    ...
    if not result.succeeded:
        return False, {"inventory_full":"背包空间不足，奖励未领取","user_missing":"角色不存在","state_changed":"战令领奖未结算：战令数据刚被改动，请重新查询","operation_conflict":"领取请求冲突，请重新发送"}.get(result.status,"活动战令奖励已领取")
    lines = ["活动战令奖励领取成功："]
...
def claim_sign(user_id: str, operation_id: str | None = None) -> tuple[bool, str]:
    ...
    if result.status == "duplicate":
        ...
        return True, "\n".join(lines)
    if not result.succeeded:
        return False, {"already_signed":"今日已经领取过活动签到","inventory_full":"背包空间不足，奖励未领取","user_missing":"角色不存在","state_changed":"签到未结算：签到数据刚被改动，请重试","operation_conflict":"签到请求冲突，请重新发送"}.get(result.status,"活动签到失败")
