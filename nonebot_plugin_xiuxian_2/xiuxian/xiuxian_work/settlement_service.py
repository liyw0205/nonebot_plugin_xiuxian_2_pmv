...
class WorkSettlementService:
    ...
    def settle(self, operation_id, user_id, expected_work, exp_gain, item, max_exp, max_goods_num) -> WorkSettlementResult:
        ...
        payload = json.dumps([user_id, expected, exp_gain, reward, max_exp, max_goods_num], ensure_ascii=True, sort_keys=True, default=str)
        ...
        with self._lock, closing(db_backend.connect(self._game_database)) as conn:
            try:
                ...
                conn.execute(
                    ...
                    "item_awarded INTEGER NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
                )
                previous = conn.execute(
                    "SELECT payload, exp, item_awarded FROM work_settlement_operations WHERE operation_id=%s",
                    (operation_id,),
                ).fetchone()
                if previous is not None:
                    ...
                    return WorkSettlementResult("duplicate", int(previous[1]), bool(previous[2]))

                user = conn.execute("SELECT COALESCE(exp, 0) FROM user_xiuxian WHERE user_id=%s", (user_id,)).fetchone()
                work = conn.execute("SELECT type, create_time, scheduled_time FROM user_cd WHERE user_id=%s", (user_id,)).fetchone()
                if user is None:
                    conn.rollback()
                    return WorkSettlementResult("user_missing", 0, False)
                if work is None or int(work[0] or 0) != 2 or tuple(map(str, work[1:])) != (
                    str(expected.get("create_time")), str(expected.get("scheduled_time")),
                ):
                    conn.rollback()
                    return WorkSettlementResult("state_changed", 0, False)
                ...
                conn.execute("UPDATE user_cd SET type=%s, create_time=%s, scheduled_time=%s WHERE user_id=%s", (0, 0, None, user_id))
                if reward is not None:
                    now = datetime.now()
                    conn.execute(
                ...
                conn.execute(
                    "INSERT INTO work_settlement_operations (operation_id,payload,exp,item_awarded) VALUES (%s,%s,%s,%s)",
                    (operation_id, payload, applied_exp, int(reward is not None)),
                )
                conn.commit()
                return WorkSettlementResult("applied", applied_exp, reward is not None)
            except Exception:
                conn.rollback()
                raise
