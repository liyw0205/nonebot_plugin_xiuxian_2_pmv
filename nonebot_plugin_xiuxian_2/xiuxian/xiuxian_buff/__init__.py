...
async def out_closing_(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    ...
    if not is_type:
        ...
        if exp >= user_get_exp_max:
            ...
            result_msg, result_hp_mp = OtherSet().send_hp_mp(user_id, int(exp * (10 * hp_speed) * (1 + mainbuffclors)), int(exp * mp_speed * 100))
...
气血:{number_to(user_msg['hp'])}/{number_to(int((user_msg['exp'] / 2) * (user_msg['hppractice'] * 0.1 + 1) * (1 + main_hp_buff + impart_hp_per)))}({((user_msg['hp'] / ((user_msg['exp'] / 2) * (user_msg['hppractice'] * 0.1 + 1) * (1 + main_hp_buff + impart_hp_per)))) * 100:.2f}%)
...
元血修炼:{user_msg['hppractice']}级(提升气血{user_msg['hppractice'] * 10}%)
