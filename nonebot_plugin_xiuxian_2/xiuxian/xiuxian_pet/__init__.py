...
def _format_pet_release_refund(pets: list[dict], refund_item: dict, total_refund_count: int):
    ...
    if len(pets) == 1 and total_refund_count > 0:
        return (
...
**砸蛋**
   ...
   > 1-10
   消耗
   > {number_to(EGG_COST)}灵石
   规则
   > 没有出战宠物时自动出战，否则进入宠物背包。
   保底
   > 累计砸蛋{EGG_PITY_THRESHOLD}次自动额外抽取1只卓越/传说/神话宠物（{_format_egg_pity_weight_text()}），连续9次保底未出神话则第10次保底必出神话。
   宠物蛋
   > 发送【道具使用 宠物蛋名 [数量]】可孵化指定稀有度宠物。

**查看宠物**
   发送
   > 我的宠物（查看当前出战宠物）
   发送
   > 宠物背包 [页码]（查看所有宠物，出战宠物排在最前，其余按品阶从高到低排序）
   发送
   > 查看宠物 宠物UID
...
**喂食升级**
   ...
   > 按一品至九品递增，品阶越高经验越多。
   规则
   > ☆主要通过喂食经验提升；当前品阶达到四个☆时需先喂满经验，再使用宠物融合突破为★。
...
**宠物启明**
   ...
   > 启明石 x1
   作用
   > 为指定宠物重新随机获得1个技能，不填UID时默认当前出战宠物。

**融合突破**
   发送
   > 宠物融合 本体UID [本体UID...] [破阶UID]
   例如
   > 宠物融合 UID1 UID2 UID3
   规则：
   - 默认以当前出战宠物作为主宠
...
**开始游历**
   ...
   > 宠物游历 灵草谷 4
   时长范围
   > {PET_TRAVEL_MIN_HOURS}-{PET_TRAVEL_MAX_HOURS}小时
   不填地点默认灵草谷，不填小时默认4小时。
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if not started.succeeded:
        await handle_send(bot, event, "游历操作未结算：游历已结束/未开始，或进度刚被改动，请重新查询。")
        return
    result_msg = "派遣成功。"
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    ...
    if claim_result.status == "state_changed":
        await handle_send(bot, event, "领取未结算：游历尚未完成，或奖励已领取/进度刚被改动，请重新查询。")
        return
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if not hatched.succeeded:
        status_msg = {
            ...
            "state_changed": "砸蛋未结算：灵石不足，或宠物背包刚被改动，请重试。",
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    token = args.extract_plain_text().strip()
    user_id = str(user_info["user_id"])
    pet_doc = get_pet_doc(user_id)
    if token:
        location, _, pet = find_pet_anywhere(pet_doc, token)
    else:
        location, pet = "active", pet_doc.get("active")
    if not pet:
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if not result.succeeded:
        messages = {"state_changed": "操作未结算：宠物或材料不足，或背包刚被改动，请重试。"}
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if ok:
        ...
        if not result.succeeded:
            result_msg = "融合失败：材料宠不存在、已锁定，或数据刚被改动，请重试。"
            pet = None
            skill_offer = None
    has_skill_offer = False
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if not result.succeeded:
        messages = {
            ...
            "state_changed": "启明失败：道具不足，或宠物数据刚被改动，请重试。",
        }
