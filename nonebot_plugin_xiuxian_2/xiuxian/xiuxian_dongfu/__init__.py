...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    ...
    msg = (
        ...
        "\n"
        "**规矩**\n"
        f"> 初有{DONGFU_PLOT_COUNT}块灵田，最多扩至{DONGFU_PLOT_MAX}块。\n"
        "> 地图渔猎采掘与探索，或得洞府资材。\n"
        f"> 巡山日限{DONGFU_PATROL_DAILY_LIMIT}次，可减被潜入之险。\n"
        "> 潜入仅可对附近已立洞府的道友。\n"
        f"> 主动潜入日限{INFILTRATE_ACTIVE_DAILY_LIMIT}次，随机潜入日限{INFILTRATE_RANDOM_DAILY_LIMIT}次；\n"
        f"> 每府日限被潜入{INFILTRATE_DAILY_LIMIT}次。"
    )
