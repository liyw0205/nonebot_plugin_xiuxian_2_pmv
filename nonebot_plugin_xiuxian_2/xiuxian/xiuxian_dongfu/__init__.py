...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    ...
    msg = (
        ...
        "\n"
        "**规则**\n"
        f"> 初始{DONGFU_PLOT_COUNT}块灵田，最多扩建至{DONGFU_PLOT_MAX}块。\n"
        "> 地图钓鱼/采集/挖矿/探索可获得洞府材料。\n"
        f"> 洞府巡山每日最多{DONGFU_PATROL_DAILY_LIMIT}次，可降低被潜入风险。\n"
        "> 只能对附近已建设洞府的道友潜入。\n"
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if result.status in {"plot_occupied", "state_changed", "dongfu_missing"}:
        await handle_send(bot, event, "灵田操作未结算：灵田等级/作物进度刚被改动，或材料不足，请重新尝试。")
        return
    d = _get_dongfu(uid)
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if result.status in {"not_mature", "state_changed", "user_missing", "dongfu_missing"}:
        await handle_send(bot, event, "种植未结算：田块已占用、种子不足，或进度刚被改动，请重新尝试。")
        return
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if not result.succeeded:
        await handle_send(bot, event, "洞府操作未结算：洞府不存在、权限不足，或数据刚被改动，请稍后重试。")
        return
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, args: Message = CommandArg()):
    ...
    if detected and not success:
        ...
        if not result.succeeded:
            await handle_send(bot, event, "潜入未结算：目标洞府防护变更、体力不足，或进度刚被改动，请稍后重试。")
            return
