...
---
**警告**
> 此操作不可逆。修为、功法、神通、灵石、修炼等级、虚神界修炼时间将被清空。
> 散尽修为后，可凝聚万世道果为极致天赋。

**进入轮回**
- 千世轮回
> 获得【轮回灵根】，最低境界：{XiuConfig().lunhui_min_level}
- 万世轮回
> 获得【真·轮回灵根】，最低境界：{XiuConfig().twolun_min_level}
- 永恒轮回
> 获得【永恒灵根】，最低境界：{XiuConfig().threelun_min_level}
- 进入无限轮回
> 获得【命运灵根】，最低境界：{XiuConfig().Infinite_reincarnation_min_level}
> 每入一次无限轮回，命运道果再增修为效率；层数愈高，单次增益渐缓，然可累加
> 闭关、修炼之灵根倍率，以永恒道果为基，再叠命运累进
...
**轮回印记**
> 每次轮回自动记录：主功法 / 辅修功法 / 神通 / 身法 / 瞳术。
> 可用【回忆前世 + 类型】取回，例如：回忆前世 主功法。
> 每种类型只能取回一次，取回后印记作废。
> 需达到轮回前境界3个大境界；无限轮回可减免（1次1个小境界）。

> 轮回后更新灵根资质；每次赠送【灵根改名卡】x1；装备和物品不会丢失。
""".strip()
...
async def resetting_(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    ...
    if user_msg['level'] in ['感气境初期', '感气境中期', '感气境圆满']:
        ...
        if not result.succeeded:
            await handle_send(bot, event, "自废修为未执行：角色状态已更新，请重新发起。")
            await resetting.finish()
        msg = f"{user_name}现在是一介凡人了！！"
...
async def confirm_lunhui_(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    ...
    if result.status == "duplicate":
        ...
        await confirm_lunhui.finish()
    if not result.succeeded:
        await handle_send(bot, event, "轮回未执行：确认状态已更新，请重新进入轮回后再确认。", md_type="轮回")
        await confirm_lunhui.finish()
...
def retrieve_reincarnation_skill(user_id, skill_type, operation_id=None):
    ...
    if result.status == "duplicate":
        ...
        return True, f"成功回忆前世中的技能：{skill_name}\n该回忆请求已经处理，无需重复提交。"
    if not result.succeeded:
        return False, "回忆前世未完成：印记状态已更新，请重新回忆"
