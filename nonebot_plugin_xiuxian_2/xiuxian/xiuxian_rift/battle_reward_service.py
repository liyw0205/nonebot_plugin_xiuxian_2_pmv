...
class RiftBattleRewardService:
    """Settle every eligible rift battle reward in one game-database transaction."""

    def __init__(self, database: str | Path, lock: RLock | None = None) -> None:
        self._database = Path(database)
        self._lock = lock or RLock()

    def award(self, operation_id, rewards, max_goods_num) -> RiftBattleRewardResult:
        ...
        with self._lock, closing(db_backend.connect(self._database)) as conn:
            try:
                ...
                conn.execute("CREATE TABLE IF NOT EXISTS rift_battle_reward_operations (operation_id TEXT PRIMARY KEY, payload TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
                previous = conn.execute("SELECT payload FROM rift_battle_reward_operations WHERE operation_id=%s", (operation_id,)).fetchone()
                if previous is not None:
                    conn.rollback()
                    return RiftBattleRewardResult("duplicate" if str(previous[0]) == payload else "state_changed")
                for user_id, _, _, _ in normalized:
                    if conn.execute("SELECT 1 FROM user_xiuxian WHERE user_id=%s", (user_id,)).fetchone() is None:
                        conn.rollback()
                        return RiftBattleRewardResult("user_missing")
                for user_id, _, _, reward_items in normalized:
                    for item_id, _, _, amount in reward_items:
                        ...
                        if (int(row[0]) if row else 0) + amount > max_goods_num:
                            conn.rollback()
                            return RiftBattleRewardResult("inventory_full")
                now = datetime.now()
                ...
                conn.execute("INSERT INTO rift_battle_reward_operations VALUES (%s,%s,CURRENT_TIMESTAMP)", (operation_id, payload))
                conn.commit()
                return RiftBattleRewardResult("applied")
            except Exception:
                conn.rollback()
                raise


__all__ = ["RiftBattleRewardResult", "RiftBattleRewardService"]
