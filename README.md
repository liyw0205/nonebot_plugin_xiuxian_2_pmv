...
绑定/删除仅管理员可用，普通用户可查看目录、发送文件和获取下载链接。
...
| `webdav列表 [序号] [路径]` | `网盘列表` | 列出 WebDAV 目录，默认第 1 个绑定和根目录；目录可点击进入，文件可点击发送 |
| `webdav信息 [序号] <路径>` | `网盘信息` | 查看文件或目录信息 |
| `webdav链接 [序号] <路径>` | `网盘链接` | 生成 WebDAV 地址，访问仍需用户名和密码 |
| `webdav文件 [序号] <路径>` | `网盘文件` | 下载并发送指定文件 |
